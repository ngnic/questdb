...
import io.questdb.cairo.WriteToQueue;
 ...
 * closed.
 */
public class WalPool extends AbstractPool {
    ...
    private static final Log LOG = LogFactory.getLog(WalPool.class);
     ...
     */
    public WalPool(CairoConfiguration configuration, @NotNull MessageBus messageBus, @NotNull Metrics metrics) {
    ...
    /**
     * <p>
     * Creates or retrieves existing WalWriter from pool. Because of WalWriter compliance with <b>single
     * writer model</b> pool ensures there is single WalWriter instance for given table name. Table name is unique in
     * context of <b>root</b> and pool instance covers single root.
     * </p>
     * When WalWriter from this pool is used by another thread @{@link EntryUnavailableException} is thrown and
     ...
     */
    public WalWriter get(CharSequence tableName, CharSequence lockReason) {
     ...
     */
    public WalWriter getOrPublishCommand(CharSequence tableName, String lockReason, WriteToQueue<TableWriterTask> writeAction) {
        while (true) {
            try {
    ...
    public void unlock(CharSequence name, @Nullable WalWriter writer, boolean newTable) {
        long thread = Thread.currentThread().getId();
        ...
        if (e.owner == thread) {
            ...
            if (newTable) {
                ...
                writer = new WalWriter(configuration, name, messageBus, null, false, e, root, metrics);
            }
    ...
    private WalWriter checkClosedAndGetWriter(CharSequence tableName, Entry e, CharSequence lockReason) {
    ...
    private WalWriter createWriter(CharSequence name, Entry e, long thread, CharSequence lockReason) {
        try {
            checkClosed();
            LOG.info().$("open [wal table=`").utf8(name).$("`, thread=").$(thread).$(']').$();
            e.writer = new WalWriter(configuration, name, messageBus, null, true, e, root, metrics);
            e.ownershipReason = lockReason;
    ...
    private WalWriter getWriterEntry(CharSequence tableName, CharSequence lockReason, WriteToQueue<TableWriterTask> writeAction) {
    ...
    private WalWriter logAndReturn(Entry e, short event) {
    ...
    private class Entry implements LifecycleManager {
        ...
        public boolean close() {
            return !WalPool.this.returnToPool(this);
        }

        public WalWriter goodbye() {
            WalWriter w = writer;
            if (writer != null) {
